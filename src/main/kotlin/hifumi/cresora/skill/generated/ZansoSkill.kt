  ...
  override fun activate(
    ...
    ; run execute@ {
      if (!WeaponSkillService.consumeTao(player, 3)) {
              ...
              3).formatted(Formatting.RED), true);
                              return@execute;
                          }
    ...
    ; run execute@ {
      ...
      if (state != null && now < state.expireTick) {
                              if (now % 30L == 0L) {
                                  ...
                                  world.getOtherEntities(player, player.boundingBox.expand(3.0)) {
              it is LivingEntity && it.isAlive }
                                      .forEach { entity ->
                                          val target = entity as LivingEntity;
                                          val baseDamage =
              WeaponCombatSupport.attackDamage(definition, data).toFloat();
                                          target.damage(world,
              world.damageSources.playerAttack(player), baseDamage * 0.5f);
                                          target.damage(world, world.damageSources.magic(),
              baseDamage * 0.5f);

                                          if (target.health / target.maxHealth < 0.10f && !((target
              as? AdventureRankMobAccess)?.cresoraIsEliteMob() ?: false)) {
                                              target.kill(world);
                                             
              player.sendMessage(Text.translatable("item.cresora.weapon.tanboku_chokuu.zanso_instakill").formatted(Formatting.DARK_RED),
              true);
                                          }
                                          world.spawnParticles(ParticleTypes.SNOWFLAKE, target.x,
              target.y + 1, target.z, 3, 0.1, 0.1, 0.1, 0.05);
                                      }
                              }
                          }
    }
