  ...
  override fun activate(
    ...
    ; run execute@ {
      ...
      val targets = world.getOtherEntities(player, player.boundingBox.expand(radius)) { entity ->
                          ...
                          val mobAccess = hostile as? hifumi.cresora.adventurerank.AdventureRankMobAccess;
      ...
      for (target in targets) {
                          ...
                          hifumi.cresora.combat.BaaMimicService.markTransformedSheep(sheep!!, target);
                          ...
                          hifumi.cresora.adventurerank.AdventureRankService.refreshMobDisplay(sheep!!);
                          target.discard();
                          transformedCount++;
                      }
      if (transformedCount > 0) {
                          player.sendMessage(net.minecraft.text.Text.translatable("item.cresora.weapon.skill.baa_mimic_activated", net.minecraft.text.Text.translatable(definition.translationKey()), transformedCount).formatted(net.minecraft.util.Formatting.GREEN), true);
                      } else {
                          player.sendMessage(net.minecraft.text.Text.translatable("item.cresora.weapon.skill.baa_mimic_activated.generic").formatted(net.minecraft.util.Formatting.GRAY), true);
                      }
    }
