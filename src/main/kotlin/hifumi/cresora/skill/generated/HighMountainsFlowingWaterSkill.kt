...
public object HighMountainsFlowingWaterSkill : WeaponSkillHandler {
  override fun activate(
    ...
    ; run execute@ {
      ...
      val pulseHealHp = hifumi.cresora.weapon.WeaponCombatSupport.secondarySkillValueHp(definition, data).coerceAtLeast(0.0f)
      val expireTick = hifumi.cresora.weapon.WeaponSkillService.currentWorldTime(player) + definition.skill.durationSeconds * 20L
      val count = hifumi.cresora.weapon.WeaponSkillService.healNearbyAllies(player, definition.skill.radiusMeters, immediateHealHp)
      val results = hifumi.cresora.weapon.WeaponSkillService.restoreHealthOrGuard(player, immediateHealHp, expireTick)
      player.sendMessage(
                          net.minecraft.text.Text.translatable(
                              ...
                              count,
                              hifumi.cresora.weapon.WeaponSkillService.formatNumber(pulseHealHp / 2.0),
  ...
  override fun onPlayerTick(
    ...
    ; run execute@ {
      ...
      if (hifumi.cresora.weapon.WeaponSkillService.isCoolingDown(player, "gaoshan_liushui")) {
                           val remaining = hifumi.cresora.weapon.WeaponSkillService.getRemainingCooldownTicks(player, "gaoshan_liushui");
                           ...
                           if (activeTicks > 0 && activeTicks <= 8 * 20 && activeTicks % 40 == 0L) {
                               val pulseHealHp = hifumi.cresora.weapon.WeaponCombatSupport.secondarySkillValueHp(definition, data).coerceAtLeast(0.0f);
                               hifumi.cresora.weapon.WeaponSkillService.restoreHealthOrGuard(player, pulseHealHp, now + 200L);
                               // Knockback
                               val radius = definition.skill.radiusMeters;
                               player.world.getOtherEntities(player, player.boundingBox.expand(radius)) { it is net.minecraft.entity.mob.HostileEntity && it.isAlive }
                                   .forEach { (it as net.minecraft.entity.mob.HostileEntity).takeKnockback(1.15, player.x - it.x, player.z - it.z) }
                           }
                      }
    }
