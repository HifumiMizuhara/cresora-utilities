...
public object ThoughtsoftheDistantBoySkill : WeaponSkillHandler {
  public val passiveStatsStates: MutableMap<UUID, ThoughtsoftheDistantBoySkill.PassiveStatsState> =
      mutableMapOf()

  public val shojoResolveStates: MutableMap<UUID, ThoughtsoftheDistantBoySkill.ShojoResolveState> =
      mutableMapOf()
  ...
  override fun onPlayerTick(
    ...
    ; run execute@ {
      val passiveState = ThoughtsoftheDistantBoySkill.passiveStatsStates.getOrPut(player.uuid) {
          ThoughtsoftheDistantBoySkill.PassiveStatsState(0L, 0) }
      passiveState.expireTick = now + 100 * 20L
      passiveState.stacks = 1
      val hasShojo = hifumi.cresora.weapon.WeaponSkillService.hasWeaponInInventory(player,
          "harukanaru_shojo_no_ketsui")
      if (hasShojo) {
                              if (hifumi.cresora.weapon.WeaponSkillService.hasMark(player,
              "nageki")) {
                                  hifumi.cresora.weapon.WeaponSkillService.removeMark(player,
              "nageki");
                                  val access = player as? hifumi.cresora.weapon.WeaponSkillAccess;
                                  if (access != null && access.cresoraGetShieldWeaponId() ==
              definition.id) {
                              ...
                              val resolveState =
              ThoughtsoftheDistantBoySkill.shojoResolveStates.getOrPut(player.uuid) {
              ThoughtsoftheDistantBoySkill.ShojoResolveState(0L, 0) };
                              resolveState.expireTick = now + 100 * 20L;
                              resolveState.stacks = 1;
                          } else {
                              hifumi.cresora.weapon.WeaponSkillService.applyMark(player, "nageki",
              10L);
              ...
              hifumi.cresora.weapon.WeaponSkillService.grantShield(player, 6.0f, 99999999L,
              definition.id);
                                              player.sendMessage(
  ...
  override fun activate(
    ...
    ; run execute@ {
      ...
      if (isGriefActive) {
                              // Pattern A
                              hifumi.cresora.weapon.WeaponSkillService.startCooldown(player,
              definition.id, 200L);
                              hifumi.cresora.weapon.WeaponSkillService.showCooldownBar(player,
              definition);
                              
                              val shieldHearts = 10.0 + (data.baseLevel - 1) * 0.5;
                              hifumi.cresora.weapon.WeaponSkillService.grantShield(player,
              (shieldHearts * 2.0).toFloat(), 99999999L, definition.id);
              ...
              player.boundingBox.expand(range)) {
                              ...
                              val playerAtk =
              player.getAttributeValue(net.minecraft.entity.attribute.EntityAttributes.ATTACK_DAMAGE);
                              val damageAmount = (playerAtk * (3.0 + data.skillLevel *
              0.5)).toFloat();
